...
async function oeffneRepo(page: Page, dtag: string): Promise<void> {
    ...
    //
    // **Kein Aufklapper mehr.** Bis zur GitHub-Parität (2026-08-27) lag das
    // README hinter einem `<details data-forge-steckbrief>`, das in der
    // schmalen Form GESCHLOSSEN startete — ein Helfer musste es vor jeder
    // Messung öffnen (`oeffneSteckbrief`, hier gestanden). Das Markup ist
    // ersatzlos gefallen: das README steht seit der GitHub-Parität direkt in
    // der Hauptspalte des Code-Reiters (`⚡forge-repo.blade.php`, Abschnitt
    // „README (P6, seit der GitHub-Parität in der HAUPTSPALTE)"), ohne
    // eigenes Sichtbarkeits-Gate — es rendert und ist sichtbar, sobald der
    // Code-Reiter (seit derselben Parität der STARTWERT von `oeffneRepo`,
    // `forge.ts: tabFromLocation()`) offen ist. `[data-forge-steckbrief]`
    // existiert nicht mehr; ein Aufruf lief in einen 30-s-Timeout und riss
    // über diesen gemeinsamen Helfer alle Tests der Datei mit.
...
test.describe('Forge: das README des Code-Browsers', () => {
    ...
    test('ein zweiter Besuch lädt NICHT erneut — der Klon liegt lokal', async ({ page }) => {
        ...
        await page
            ...
            .first()
            .click()
        // Kein `oeffneSteckbrief` mehr nötig — das README steht direkt im
        // Code-Reiter, dem seit der GitHub-Parität STARTWERT dieser Route.
        await expect(page.locator('[data-forge-readme]')).toHaveAttribute('data-lage', 'da', { timeout: 60_000 })
