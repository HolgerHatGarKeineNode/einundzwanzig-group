...
| **B5** — Favicon / OG / Head dynamisch | | ✅ **fertig** | Server-seitige NIP-11-Space-Identität via `SpaceCache::relayInfo()`/`refreshRelayInfo()` (Read-Through-Cache, vom `nostr:warm-cache`-Command gewärmt). Space-Seite: Titel = Relay-`name` (Fallback „Space"). Per-Raum/-Space **OG-Bild**: Raum-`picture` bzw. Space-`icon` (NIP-11), absolut über den Bild-Proxy (neues `og`-Preset 1200×630/cover), Fallback `og.png`; `head.blade.php` `og:image`/`twitter:image` dynamisch (`$ogImage ?? asset('og.png')`). Directory teilt das Space-`icon`-OG. **Favicon bleibt App-fix** (YAGNI, Single-Space rechtfertigt Nostr-Favicon nicht). Portal-Head unberührt (Host regelt selbst). Tests: +4 Pest (`refreshRelayInfo` NIP-11, Space-Titel/-OG, Fallback, per-Raum-OG) grün. |
| **B6** — Weitere Nostr-Metadaten (Backlog) | Kür/Ideen | ✅ **umgesetzt** (Auswahl) | Zwei Items gezogen: **NIP-11-`banner`** → breites Space-Kopfbild über dem Header (proxifiziert, neues `banner`-Preset 3:1/cover, Fade nach unten, nur wenn Relay eins liefert) und **Custom-Emoji NIP-30** → `:shortcode:` + `emoji`-Tag rendert als zeilenhohes Inline-`<img>` über den Proxy (welshmans `renderEmoji` ist nicht per Option überschreibbar → Emoji-Nodes im `parse`-Walk selbst zu `<img>`, nur https; Rest an welshman). `banner`/`website`/`lud16` liefen schon über B3. Tests: spaceBranding-`banner` (2) + `banner`-Preset→WebP (1) + E2E `room` Custom-Emoji (Inline-`img` über `/img/avatar`) grün. **Bewusst offen** (Backlog): `livekit` (out of scope), `pictureMeta`/blurhash, NIP-30 nur wenn im Vereins-Chat real genutzt. |
...
## B5 — Favicon / OG / Head dynamisch — ✅ umgesetzt

> **Ist-Umsetzung (2026-07-09):** Server-seitige Space-Identität ergänzt: `SpaceCache::relayInfo()` liest den NIP-11-Cache, `refreshRelayInfo()` holt das NIP-11-Dokument (`Accept: application/nostr+json`, `ws→http`-Rewrite, `name`/`description`/`icon`) — im selben Read-Through-Muster wie die Räume, gewärmt vom `nostr:warm-cache`-Command (nie im Request-Pfad). **Titel:** Space-Seite = Relay-`name` (Fallback „Space"), `#[Title]`-Attribut → `render()->title()`; Directory bleibt „Mitglieder" (fixer Abschnittsname). **OG-Bild:** neues `og`-Preset (1200×630/cover) im `ImageProxyController`; die drei SFCs teilen per `View::share('ogImage', …)` ein **absolutes** proxifiziertes Bild — Raum-Seite: Raum-`picture`, sonst Space-`icon`; Space+Directory: Space-`icon`. `head.blade.php` rendert `og:image`/`twitter:image` aus `$ogImage ?? asset('og.png')` (parallel zur bestehenden `$pageTitle`-Zeile). **Favicon bleibt App-fix** (bewusst YAGNI — Single-Space rechtfertigt kein Nostr-Favicon). Portal-/Package-Head unberührt (Host regelt selbst). Generiertes Marken-OG (Icon+Raumname) bewusst nicht gebaut (Kür/L). Tests: 4 neue Pest (`refreshRelayInfo` NIP-11-Parsing/Cache, Space-Titel+`og`-OG, Fallback „Space"+`og.png`, per-Raum-OG aus `picture`) grün.
...
## B6 — Weitere Nostr-Metadaten (Backlog / bewusst optional) — ✅ Auswahl umgesetzt

> **Ist-Umsetzung (2026-07-09):** Zwei Items aus dem Backlog gezogen (Rest bleibt bewusst offen):
> - **NIP-11 `banner` → Space-Kopfbild:** welshman parst `banner` bereits ins `RelayProfile`. Durchgereicht via `spaceBranding()` (relayCaps.ts) → `SpaceView.banner` (groups.ts, fließt über `...brand`) → `⚡spaces.blade.php`: `<template x-if="space?.banner">` rendert ein breites Kopfbild über dem Header (`$img(space.banner, 'banner')`), neues **`banner`-Preset** (1200×400/`cover`) im `ImageProxyController`, Fade nach unten hält den Header lesbar, dekorativer einstufiger `onerror` (Bild weg statt Chip). Erscheint nur, wenn der Relay ein `banner` liefert.
> - **Custom-Emoji NIP-30:** welshmans `renderEmoji` ist **nicht** per `RenderOptions` überschreibbar (fest in `renderOne`). Darum in `feeds.ts` den `parse`-Walk selbst gemacht: Emoji-Nodes (`ParsedType.Emoji`, `value.name`/`value.url`) mit **https**-URL → eigenes Inline-`<img class="chat-emoji">` über den Proxy (`avatar`-Preset, zeilenhoch via `theme.css`), alle anderen Nodes weiter an `renderAsHtml` (Text-Escaping, Links, Newlines). Nicht-https/kein-URL → welshman-Default (Text-Shortcode).
> - `banner`/`website`/`lud16` (Lightning) waren bereits über **B3** (Profil-Karte) abgedeckt.
> - Tests: 2 Logik (`spaceBranding` mit `banner`) + 1 Pest (`banner`-Preset → WebP) + E2E `room` (Custom-Emoji → Inline-`img` über `/img/avatar`); volle `room`-Suite 17/17 grün, `npm run build` + `tsc --noEmit` grün.
> - **Bewusst NICHT gebaut** (Backlog bleibt): `RoomMeta.livekit` (out of scope), `pictureMeta`/blurhash-Platzhalter, NIP-11 `limitation`-Hinweise. NIP-30 nur aktiviert, weil billig — real genutzt wird's im Vereins-Chat erst noch.
