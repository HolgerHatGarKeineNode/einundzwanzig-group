...
| **M1** — Nostr-Login (NIP-07/46/nsec) | ✅ **fertig** | Client-Login (nsec/NIP-07/NIP-46 + welshman-Session + localStorage) **und** NIP-98-Handoff an die Laravel-Session (server-seitige Schnorr-Verifikation via `swentel/nostr-php`, Einmal-Nonce) + `nostr.auth`-Gate (`/spaces`) + beidseitiges Logout. 8 Feature-Tests grün. **Amber via `nostrconnect://`-QR** (dritter Login-Tab, `loginWithNostrConnect` → `qrcode`-DataURL, Broker-`waitForNostrconnect`) im Web vorgezogen; `hasExtension`-Erkennung pollt jetzt ~3 s (Alby/nos2x injizieren `window.nostr` async nach Alpine-init). +2 E2E (Amber-QR-Render, verzögerte Extension). **NIP-55-Deeplink (same-device mobil) → M8**. |
...
| **M5** — Chat senden + Raum join/leave | ✅ **fertig** | Schreib-Primitive in `feeds.ts`/`groups.ts`: senden (kind 9, optimistisch übers Repository), eigene Nachricht löschen (kind 5, Tombstone-`created_at` gebumpt), **echtes relay-seitiges Beitreten/Verlassen** (NIP-29 kind **9021/9022** → zooid genehmigt offene Räume automatisch und pflegt die relay-signierte Members-Liste **39002**; Mitgliedschaft ist damit persistent und übersteht Reloads). `/rooms/{h}`: Composer **nur für Mitglieder** (an 39002 gekoppelt, `membershipReady` gegen Aufblitzen), sonst Beitreten-Hinweis; „Verlassen" im Header; Löschen bei eigenen Nachrichten. Meine/Andere Räume = 39002-Mitgliedschaft. Reply → M6. 3 E2E-Tests (senden, löschen, **beitreten→Reload-persistent→verlassen**); Suite 14/14. |
| **M5.5** — Livewire-Full-Page-SFCs + `wire:navigate` | ✅ **fertig** | Alle Nostr-Seiten (home, spaces, directory, room, nostr-login, nostr-smoke, settings/space) von eigenständigen `<html>`-Blades auf **Livewire-v4-SFCs** (`resources/views/pages/⚡*.blade.php`, `Route::livewire`) mit gemeinsamem Layout `layouts::einundzwanzig` migriert. Die welshman/Alpine-Inseln bleiben unverändert (`x-data`), Server→Insel-Übergabe via `@js($prop)`. `wire:navigate` überall → **welshman-Singleton bleibt warm** (JS-Heap überlebt Navigation, E2E-verifiziert), instant Nav, optimistischer State übersteht Seitenwechsel. Server-Seam für späteren Read-Cache (§10/M7). Suite 14/14. |
| **M6** — Politur | ✅ **fertig** | Signer-Health-Banner (welshman `signerLog` → app-weit „Signer antwortet nicht"), Publish-Fehler als Flux-Toasts (Inline-Callout ersetzt), **Reply/Quote** (kind-9 `q`+`p`+`nostr:nevent`, Zitat-Vorschau im selben Raum), **Admin/NIP-86** (`manageRelay`: Rollen anlegen/bearbeiten/löschen/zuweisen, Member entfernen/bannen/entbannen, Gebannt-Liste, Invite-Link + `/join`-Einlösen; `deriveUserIsSpaceAdmin` mit expliziter Invalidierung = **Fix C**), **Space-Join** (28934/28936 + 10009), **Bottom-Nav** (`flux:navbar`, §12), **CSP**-Middleware + OG-Meta, **Vendor-Chunking** (welshman-Insel 769 KB → eigener cache-stabiler Chunk, App-Bundle 12 KB). 17 Playwright-E2E + 43 Pest grün. |
| **M7** — Realtime/Backend (optional) | ✅ **fertig** | Read-Cache-Slice: `nostr:warm-cache` (Scheduled) liest kind 39000 via `swentel/nostr-php` in einen Laravel-Cache (Read-Through, nie autoritativ, nie Key-Kontakt); Room-SFC rendert Titel/Header/`og:description` server-seitig daraus (First-Paint + per-Raum-OG), Cache-Miss → Raum-ID-Fallback. `app/Nostr/SpaceCache.php`, kein neuer Dep. Realtime-UI = welshman-Live-Subs (kein WS-Server, per Constraint). Notifications-Persistenz bewusst → M8 (Push). 3 Pest-Tests. Commit `5300dc8`. |
| **M8** — NativePHP Mobile (Release) | ⬜ offen → **siehe `PLAN2.md`** | **Strategie-Pivot (2026-07-07):** Chat wird **nicht** eigene App, sondern **Package-Tab in der Portal-App** (`space.einundzwanzig.mobile`, Vollbild-Takeover, getrennter Nostr-Login, Composer-/npm-Package + Subtree-Split). Details, überholte §11-Annahmen & Fahrplan in **`PLAN2.md`**. Vorarbeit (Setup/Smoke) via M0.5 erledigt. |
...
- **Auth = Nostr-Login** (npub/Signer). **Signing bleibt IMMER client-seitig** — der Server sieht nur den (verifizierten) pubkey. (Plain-nsec-Login ist die Ausnahme: unsicher, nur für Tests — der Key liegt im localStorage.)
...
### M5 — Chat senden + Raum join/leave ✅
- **Ziel:** Schreiben.
- **Umsetzung:** `feeds.ts`: `sendRoomMessage` = `makeEvent(9,{tags:[['h',h]]})` → `publishThunk({relays:[url]})` (Thunk schreibt optimistisch ins Repository → Live-Sub bestätigt); `deleteRoomMessage` = kind-5-Tombstone (`['k','9'],['e',id],['h',h]`, `created_at` gebumpt), Fehler je über `waitForThunkError`. `groups.ts` (**echte NIP-29-Mitgliedschaft, relay-autoritativ**): `joinRoom`/`leaveRoom` publizieren kind **9021/9022** ans Space-Relay; zooid genehmigt offene Räume automatisch (`OnEventSaved` → `AddMember` + `UpdateMembersList`) und signiert die addressable Members-Liste **39002** (`d`=h, `p`=Mitglieder). `roomMembersByUrl` (Map<url, Map<h, Set<pubkey>>>) + `deriveUserInRoom` lesen sie reaktiv; `activeSpaceView`/`buildSpaceView` teilen Meine/Andere Räume nach 39002. `loadSpaceRooms` lädt 39002 mit; `listenRoomMembers` (Live-Sub) reflektiert Join/Leave sofort. `nostrRoomChat`: `joined`/`joining`/`membershipReady` + `send`/`remove`/`join`/`leave`. `⚡room.blade`: Composer **nur bei `membershipReady && joined`** (Skeleton bis geladen), sonst Beitreten-Hinweis; „Verlassen" im Header. Tote 10009-Raum-Helfer entfernt (die 10009 trägt nur noch die SPACE-Ebene für die Space-Auswahl).
- **DoD erfüllt:** Senden erscheint im eigenen Feed + round-trippt (E2E); Löschen entfernt die eigene Nachricht (E2E); **Beitreten/Verlassen ändert die relay-seitige Mitgliedschaft und übersteht einen Reload** (E2E prüft genau das). 3 E2E-Tests grün, Suite **14/14**.
- **Fallstricke gelöst:** (1) **Erst-Design (10009) war falsch** — die persönliche 10009-Folgeliste ist kein Schreibrecht **und** zooid (Gruppenrelay) speichert sie gar nicht → „Beitreten" überlebte keinen Reload, und ein davon entkoppelter Dauer-Composer wirkte widersprüchlich. Fix: echte relay-seitige NIP-29-Mitgliedschaft (39002), Composer daran gekoppelt. (2) Delete-`created_at` muss `> Nachricht` sein (Repository `_isDeleted`) → auf `max(now, msg+1)` gebumpt. (3) Kein `type=submit` am Senden-Button (Flux-`wire:loading`-Dauerspinner). (4) Voraussetzung fürs Testen: der Seed lässt den Test-User via 9021 `welcome`+`general` beitreten (`dev` bleibt frei für den Join/Leave-Test).
- **Nicht in M5:** **Reply/Quote** → M6. Space-Join (RELAY_JOIN 28934) + `send_delay`/ThunkToast → M6.

### M5.5 — Livewire-Full-Page-SFCs + wire:navigate ✅
- **Ziel:** Fundament für SPA-Navigation + spätere server-seitige Vorteile (Read-Cache, First-Paint), ohne die client-seitige Nostr-Architektur aufzugeben.
- **Umsetzung:** Jede Nostr-Seite ist jetzt eine **Livewire-v4-SFC** unter `resources/views/pages/⚡*.blade.php` (Konvention des Starter-Kits, `Route::livewire('…','pages::…')`), die Klasse ist ein **dünner Shell**; der reaktive Zustand bleibt komplett in der welshman/Alpine-Insel. Gemeinsames Layout `resources/views/layouts/einundzwanzig.blade.php` (`#[Layout('layouts::einundzwanzig')]`) hält `<html>/<head>/<body>` + `@fluxScripts`; die welshman-Insel wird EINMAL im `<head>` (`@vite`) geladen. **Server→Insel-Übergabe nur via `@js($prop)`** (z.B. `$h` im Raum). Alle Intra-App-Links auf `wire:navigate` (Header-Zurück, Space-Aktionen, Home-CTAs), programmatische Nav auf `Livewire.navigate(...)` (Raum-Klick, Space-Wechsel). Auth-Grenzen (Login-Handoff, Logout) bleiben bewusst Hard-Nav.
- **Gewinn (E2E-verifiziert):** Ein JS-Heap-Marker überlebt `wire:navigate` (`/spaces → /rooms/welcome`) → **welshman-Repository/Subscriptions/optimistischer State bleiben warm**, Navigation instant, kein Re-Fetch. Die Alpine-Insel re-initialisiert auf dem getauschten Body automatisch (Livewire v4). Suite **14/14**.
- **Eleganz-Entscheidung:** Kein `wire:ignore` an den Inseln, solange die SFCs inert sind (keine wire-Actions → kein Morph → kein Alpine-Konflikt). Erst wenn eine Seite echte Live-Livewire-Updates bekommt (`wire:poll` für Cache-Daten), kapselt man den reinen Alpine-Teilbaum in `wire:ignore` (dokumentierte Eskalation, YAGNI). `nostr-smoke` behält sein bestehendes `wire:ignore`.
- **Nächster Nutzen (später, §10/M7):** `mount()`/computed-Props der SFCs sind jetzt der saubere Ort, um gecachte Space-/Member-Daten server-seitig zu laden und via `@js(...)` als First-Paint an die Insel zu geben — **Server bleibt reiner Read-Through-Cache, nie autoritativ, nie Key-Kontakt.**

### M6 — Politur ✅
- **Ziel:** Produktionsreife des Kerns.
- **Umsetzung:**
  - **Signer-Health** (`signer-health.ts`): abgeleitet aus welshmans globalem `signerLog` (jede Signer-Operation, inkl. NIP-42-AUTH, hinterlässt `{started_at, finished_at?, ok?}`). Schwellen 1:1 aus dem Referenz-Client (`disconnected`/`slow`), 2 s getickt (hängender Signer wird auch ohne neues Event sichtbar). Globaler Banner (`nostrSignerBanner`) im Layout → app-weit „Signer antwortet nicht", besonders im Raum relevant.
  - **Thunk-Toasts** (`toast.ts`): Publish-Fehler (Relay-Reject/AUTH) als Flux-Toast (via `toast-show`-Document-Event, kein Livewire-Kontext nötig) statt Inline-Callout. `send_delay` bewusst **weggelassen** — es würde den optimistischen Echo verzögern und brächte ohne „Unsend"-UI nur Nachteil (YAGNI).
  - **Reply/Quote** (`feeds.ts`): kind-9-Antwort nach NIP-18-Manier — `q`+`p`-Tags + vorangestelltes `nostr:nevent…` (kein NIP-10 e-reply, wie im Referenz-Client). Beim Rendern wird der Prefix getrimmt (`bodyWithoutQuote`) und stattdessen eine kompakte **Zitat-Vorschau** gezeigt, im selben Raum aufgelöst (Cross-Relay-Auflösung = YAGNI). Composer hält den Reply-Kontext (`setReply`/`clearReply`).
  - **Admin/NIP-86** (`members.ts` + Directory-Modals): `manageRelay`-Wrapper für Rollen (`createrole`/`editrole`/`deleterole`/`assignrole`/`unassignrole`; kind 33534, HSL-Farbe) und Member (`allow`/`unallow`/`ban`/`unban`, `listbannedpubkeys`). `deriveUserIsSpaceAdmin` via `supportedmethods` mit **expliziter Invalidierung** (`refreshSpaceAdmin` nach jeder Mutation + Live-Sub auf 13534/33534 = **Fix C**, kein `memoize`-Stale). Flux-Modals (client-seitig via `modal-show`-Event): Rollen anlegen/verwalten, Rollen zuweisen (Toggle), Gebannt-Liste (ent-/wiederaufnehmen), Invite-Link generieren. Per-Member-Dropdown (Rollen/Entfernen/Bannen), nur für Admins (`isAdmin`-Gate).
  - **Space-Join** (`groups.ts`): `joinSpace` (kind 28934 + optionaler Invite-`claim`) + Aufnahme in die 10009 (`addToListPublicly`.reconcile(nip44)); `leaveSpace` (28936 + Entfernen). `parseInviteLink`/`loadSpaceInviteClaim` (28935). Einlösen über **`/join?r=&c=`** (`nostrInvite` → beitreten → aktiver Space). Beitreten/Verlassen des aktiven Space in den Einstellungen.
  - **Bottom-Nav** (`<x-bottom-nav>`, `flux:navbar`): Räume/Mitglieder/Einstellungen, fixiert, in der max-w-md-Spalte (§12, mobile-first, skaliert auf Desktop). Redundante Header-Icons entfernt.
  - **Produktionshärtung:** `ContentSecurityPolicy`-Middleware (Defense-in-Depth: `object-src none`, `base-uri`/`form-action self`, `frame-ancestors none`; `connect-src` breit für Nostr; `script-src` bleibt `unsafe-eval`/`-inline` — Alpine/Flux-Realität, dokumentiert). OG-/Twitter-Meta marken-/routenweit im `<head>` (per-Raum-Namen → Read-Cache §10/M7). Auf Mobile (`nativephp-internal.running`) beides aus.
  - **Ladeperformance:** welshman + nostr-tools (~769 KB) via `manualChunks` in einen **eigenen, cache-stabilen Vendor-Chunk** getrennt → App-Bundle 12 KB (ändert sich oft), SDK-Chunk 279 KB gzip (fast nie) = bessere Cache-Trefferquote über Deploys. Lazy-Loading der Insel selbst bewusst **nicht** — die Kern-Flows (Chat/Spaces) brauchen welshman sofort; das Warmhalten via `wire:navigate` (M5.5) ist der eigentliche Gewinn.
- **DoD erfüllt:** Verein-Kern bedienbar (Reply/Admin/Join), Admin-Aktionen (Roles/Ban) funktionieren relay-autoritativ (E2E: Owner sieht NIP-86-Tools, User nicht), keine Race/Flacker-Zustände. 3 neue E2E (Reply, Admin-Gating ×2) + bestehende → **17 Playwright-E2E + 43 Pest grün**.
- **Bewusst später:** Persistenter „Relay-AUTH-verweigert"-Callout (wie `SpaceAuthError` des Referenz-Clients) — Signer-Banner + Publish-Toasts decken den aktionablen Fall; per-Entity-OG + First-Paint-Read-Cache → M7; Rollen-**Mutations**-E2E (poll den Test-Relay-State, würde ihn zumüllen) manuell/M7.
