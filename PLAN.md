...
| **M5** — Chat senden + Raum join/leave | ✅ **fertig** | Schreib-Primitive in `feeds.ts`/`groups.ts`: senden (kind 9 via `makeEvent`+`publishThunk`, optimistisch übers Repository), eigene Nachricht löschen (kind 5, Tombstone-`created_at` gebumpt wegen Repository-Regel `> Nachricht`), Raum folgen/entfolgen (10009 via `addToListPublicly`/`removeFromListByPredicate`, optimistisch → verschiebt Meine/Andere Räume). `/rooms/{h}`: Composer (immer verfügbar, Schreibrecht erzwingt der Relay, Fehler im Callout), Löschen bei eigenen Nachrichten, Beitreten/Verlassen im Header. Reply verschoben nach M6. Composer ohne `type=submit` (Flux-Submit-Spinner braucht Livewire). 3 E2E-Tests (senden, löschen, join/leave); Suite 14/14. **Hinweis:** zooid (Gruppenrelay) lehnt kind-10009 ab — die persönliche Folgeliste gehört auf die User-Relays; hermetisch erscheinen daher alle Räume als „Andere Räume". |
...
- **Ziel:** Schreiben.
- **Umsetzung:** Schreib-Primitive neben die Lese-Ableitungen gelegt. `feeds.ts`: `sendRoomMessage(url,h,content)` = `makeEvent(9,{tags:[['h',h]]})` → `publishThunk({relays:[url]})` (der Thunk schreibt das Event optimistisch ins Repository → die Live-Sub bestätigt es); `deleteRoomMessage(url,h,id,createdAt)` = kind-5-Tombstone (`['k','9'],['e',id],['h',h]`), Fehler je über `waitForThunkError`. `groups.ts`: `joinRoom`/`leaveRoom` pflegen die persönliche **10009** (`addToListPublicly`/`removeFromListByPredicate` → `nip44EncryptToSelf` → `publishThunk`); optimistisch, `userGroupList`→`activeSpaceView` verschiebt den Raum live zwischen Meine/Andere Räume. `nostrRoomChat` erweitert (draft/sending/error/joined + `send`/`remove`/`join`/`leave`); `room.blade`: Composer (immer sichtbar), Löschen-Button bei eigenen Nachrichten, Beitreten/Verlassen im Header, Fehler-Callout.
- **DoD erfüllt:** Senden erscheint sofort im eigenen Feed (E2E) und round-trippt über zooid; Löschen entfernt die eigene Nachricht (E2E); join/leave ändert Mitgliedschaft + Sichtbarkeit (optimistisch, E2E). 3 E2E-Tests grün, Suite **14/14**.
- **Fallstricke gelöst:** (1) Composer-Gate auf 10009 war falsch — die 10009 ist eine persönliche Folgeliste, **kein** Schreibrecht; zooid speichert sie zudem nicht (Gruppenrelay). Composer daher immer offen, Schreibrecht erzwingt der Relay. (2) `deleteRoomMessage`-`created_at` muss `> Nachricht` sein (Repository-Regel `_isDeleted`), sonst greift Löschen in derselben Unix-Sekunde nicht → auf `max(now, msg+1)` gebumpt. (3) Kein `type=submit` am Senden-Button: Flux hängt Submit-Buttons einen `wire:loading`-Spinner an, den ohne geladenes Livewire niemand ausblendet (Dauer-Spinner).
- **Nicht in M5:** **Reply/Quote** bewusst nach M6 verschoben (Thread-Rendering + Parent-Auflösung verdoppeln die UI-Fläche; flacher Vereins-Chat kommt vorerst ohne aus). Space-Join (RELAY_JOIN 28934) + `send_delay`/ThunkToast ebenfalls M6.
