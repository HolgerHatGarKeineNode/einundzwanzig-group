...
<main class="mx-auto flex min-h-screen max-w-sm flex-col justify-center px-6 pt-safe pb-safe">
    <div x-data="nostrAuth" class="empty-state text-center">
        ...
        <div class="mt-6">
            ...
            <div class="mt-1 font-mono text-xs tracking-wide text-muted">{{ __('Die Bitcoin-Community auf Nostr') }}</div>
        </div>
        ...
        <template x-if="!pubkey">
            <div class="mt-8">
                <flux:button variant="primary" class="w-full" icon="bolt" :href="route('group.nostr-login')" wire:navigate>{{ __('Anmelden') }}</flux:button>
