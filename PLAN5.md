...
| **C2** — Delete eigene Nachricht (NIP-09 kind 5) + Report (NIP-56 kind 1984) | nach C0, mittel | ✅ erledigt | Delete-Pipeline existierte (`deleteRoomMessage`/`askDelete`/`confirmDelete` + `delete-message`-Modal) → **ins „…"-Menü eingehängt** (Web-Dropdown `x-if="m.mine"` + natives Modal `x-show="menuFor?.mine"`); Inline-Trash bleibt. Report: `makeReport` (kind 1984, `["p",pk]`+`["e",id,reason]`, **kein `h`/PROTECTED**) + `sendReport` (publishThunk→`mapRelayError`, kein optimistischer Feed-Eintrag) + `askReport`/`confirmReport` (Busy-Guard, Modal bleibt bei Fehler offen). `report-message`-Modal: `flux:select` Grund (NIP-56: spam/illegal/nudity/profanity/impersonation/other, Default spam) + `flux:textarea` Freitext. Melden nur bei `!m.mine`. **Monitoring:** kind-1984 landet auf zooid, Admin fragt per `nak req -k 1984 --auth` ab (verifiziert); Moderations-UI = C8. E2E grün: Löschen-über-Menü→kind-5; Report→kind-1984 (`p`+`e,reason`, kein `h`); natives Modal Löschen/Melden nach `m.mine`. Pest: Blade-Guards beide Laufzeiten. Adversariale Review (7 Agenten) + code-simplifier: 0 bestätigte Defekte. |
