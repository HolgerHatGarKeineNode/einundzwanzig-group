...
| **C1** — Emoji-Reactions (NIP-25 kind 7, Custom-Emoji NIP-30) + Toggle-Delete | nach C0, hoch | ✅ erledigt | `interactions.ts`: `makeReaction` (kind 7, `tagEventForReaction`+`h` vom Parent+PROTECTED cond.) + `makeEventDelete` (kind 5 Toggle, `created_at`>Ziel). `feeds.ts`: `sendReaction`/`removeReaction` (optimistisch, Reject-Rollback), `aggregateReactions`→`ChatMessage.reactions` (dedupe pro Autor+Emoji, `count`/`mine`), `listenRoom` erweitert auf kinds 9/7/5, `loadRoomReactions`. `bridge.ts`: `react`/`toggleReaction`. UI: Chip-Reihe + Web-Popover (Alpine, nicht `flux:dropdown` — verschluckt rohe Kinder) + Mobile-Modal-Reihe über wiederverwendbare `<x-group::reaction-picker>`. Picker = Standard-Set; Custom-Emoji (NIP-30) rendern+toggeln voll, als Picker-Quelle vertagt (kein def. Space-Emoji-Quell). **Test-Isolation:** dedizierter geseedeter `react`-Raum (kein `welcome`-Bloat). E2E grün: kind-7 e/k/h/PROTECTED + Chip + Toggle→kind-5; Custom-Emoji-Chip Inline-`<img>`; native Modal. |
| **C2** — Delete eigene Nachricht (NIP-09 kind 5) + Report (NIP-56 kind 1984) | nach C0, mittel | ✅ erledigt | Delete-Pipeline existierte (`deleteRoomMessage`/`askDelete`/`confirmDelete` + `delete-message`-Modal) → **ins „…"-Menü eingehängt** (Web-Dropdown `x-if="m.mine"` + natives Modal `x-show="menuFor?.mine"`); Inline-Trash bleibt. Report: `makeReport` (kind 1984, `["p",pk]`+`["e",id,reason]`, **kein `h`/PROTECTED**) + `sendReport` (publishThunk→`mapRelayError`, kein optimistischer Feed-Eintrag) + `askReport`/`confirmReport` (Busy-Guard, Modal bleibt bei Fehler offen). `report-message`-Modal: `flux:select` Grund (NIP-56: spam/profanity/impersonation/other, Default spam — Illegal/Nacktheit bewusst raus) + `flux:textarea` Freitext. **UI-Label = „Fork off!"** (nicht „Melden" — Plebs-Ton; NIP-56/`report` bleibt der technische Identifier), nur bei `!m.mine`. **Monitoring:** kind-1984 landet auf zooid, Admin fragt per `nak req -k 1984 --auth` ab (verifiziert); Moderations-UI = C8. E2E grün: Löschen-über-Menü→kind-5; Fork off!→kind-1984 (`p`+`e,reason`, kein `h`); natives Modal Löschen/Fork off! nach `m.mine`. Pest: Blade-Guards beide Laufzeiten. Adversariale Review (7 Agenten) + code-simplifier: 0 bestätigte Defekte. |
| **C3** — Edit (eigene, <5 min) + Quote/Share | nach C1, mittel | ✅ erledigt | Edit = Delete-des-Alten (fire-and-forget kind-5) + Re-Publish mit gleichem `created_at` (`editRoomMessage` in feeds.ts; q/p-Tag + nostr:nevent-Präfix bleiben erhalten). `canEdit(m)` = eigener pk & ≤5 min (client). Quote/Share = **Quote-Only im selben Raum** (Auftraggeber-Entscheid): `share()` nutzt den Reply-Mechanismus mit erlaubtem LEEREM Body → `sendRoomMessage('' , target)` erzeugt `nostr:nevent…\n\n` + q+p; die bestehende Zitat-Vorschau rendert ihn (kein neuer Render-Pfad). Menü „Bearbeiten"(canEdit)/„Zitieren" Web+native; Compose-Kontextbox edit/share/reply; Senden bei leer+sharing aktiv. `setReply` nimmt editingId+sharing zurück (Review-Fix). E2E grün (Bearbeiten gleiche created_at, >5-min-Grenze, Reply-Edit q/p-Erhalt, Quote-Only, native Modal); Pest Blade-Guards; adversariale 4-Lens-Review + code-simplifier eingearbeitet. |
| **C4** — Mentions (NIP-08/NIP-27) + Copy nevent/npub/JSON + Nachricht-Info | nach C0, klein | ✅ erledigt | `interactions.ts`: `mentionPubkeys` (pure npub/nprofile-Extraktion). `feeds.ts`: `withMentionTags` hängt `["p",pk,url]` an Message+Edit (dedupe gg. Reply-p), `renderMessageHtml` löst Profil-Node zu `@Name` (**Plan-Lücke: welshman-Default rendert nur gekürztes `nprofile…`** — Cache-Skip für Mention-Nachrichten, da Name async nachlädt), `warmProfiles` zieht auch Mention-Pubkeys (extern referenzierte). `bridge.ts`: `@`-Autocomplete (`onComposerInput`/`pickMention`/`closeMentions`, Quelle = 13534-Space-Directory via `loadSpaceDirectory`+`watchSpaceDirectory` im Raum-Setup, Profile gewärmt), `copyNevent`/`copyNpub`/`copyJson`/`openInfo` (`neventFor`-Helper, `tracker.getRelays`→Seen-On, Fallback Space-Relay). Blade: Mention-Popover überm Composer (Pfeil/Enter/Tab/Escape-keydown), Copy/Info-Einträge Web-Dropdown + Mobile-Modal, `message-info`-Modal (nevent/npub/Roh-JSON/Seen-On). Copy nur `navigator.clipboard` (Web+WebView) — **nativer Share-Sheet-Fallback vertagt** (kein Bedarf). E2E grün: Mention→npub+p-Tag+@Name-Render, Senden-schließt-Popover, Copy-Clipboard, Info-Modal, native Modal. Pest Blade-Guards. Adversariale 5-Lens-Review (9 Agenten): 2 low-Defekte gefixt (`closeMentions()` in `send()`; Mention-Pubkeys warmen) + code-simplifier (`neventFor`-DRY). |
...
**Ziel:** Eigene Nachricht löschen (im Menü, mit Bestätigung); fremde Nachricht anprangern (UI-Label „Fork off!", NIP-56-Report).
