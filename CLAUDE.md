...
welshman deckt Flotillas zooid-Stack bereits vollständig ab (tracker/Event→Relay-Herkunft, relay-signiertes Directory via 13534/33534, NIP-29-Groups, NIP-86-Relay-Management, NIP-42 AUTH). Deshalb wird der bestehende Nostr-Layer aus Flotilla (`src/app/repository.ts`, `groups.ts`, `members.ts`, Signer/Session) portiert statt neu geschrieben. Referenz-Quelle: `/home/user/Code/flotilla`.

- **`@welshman/store` und `@welshman/app` hängen an `svelte`** — aber nur an `svelte/store` (Runtime, kein Compiler, keine `.svelte`-Dateien). `svelte` als npm-Dep installieren; nur `svelte/store` importieren.
