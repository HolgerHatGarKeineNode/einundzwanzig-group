...
| **D4** — A11y & Responsive | | ✅ **fertig** | `.pressable:focus-visible`-Focus-Ring, `.text-muted`-Utility (zinc-600/400, alle ~20 `text-zinc-500`-Stellen umgestellt), `.icon-btn-touch` (44px nur auf `pointer:coarse`) an Chat-Aktionen, `.nav-pill` in `reduced-motion`, Modal-Fokus-Rückgabe zentral in `dispatchModal` (nativer `<dialog>`-`close`), hartes `class="dark"` aus `einundzwanzig.blade.php` raus → `@fluxAppearance` steuert das Theme, Theme-Switch (`$flux.appearance`, Hell·Auto·Dunkel) auf dem Einstellungen-Tab, Spalte auf Desktop verbreitert (`md:max-w-lg lg:max-w-2xl`, ohne Auth-Screens). Desktop-Split bewusst ausgelassen. Pest (Layout-Enthärtung + Switch + `text-muted` + Breakpoints, 4) + E2E (Switch schaltet `.dark`/persistiert, überlebt Navigation, 2) grün; volle Suite 60 Pest + 30 E2E. **Offen (anderes Repo):** Portal-`mobile.blade.php` muss ebenfalls enthärtet werden, sonst überschreibt die Portal-Shell den geteilten Store. |
| **D5** — Navigation, Landing & globale Flows | | ✅ **fertig** | Logout + Brand-Mark-Header (kein `:back`) auf Einstellungen-Tab, Directory-Header ohne `:back`, Marken-Fehlerseiten (`errors/{404,500,503}` + schlanke `errors/layout` ohne welshman/Flux-Runtime, Rückweg zur Startseite), OG-Share-Bild (`public/og.png` 1200×630 + `summary_large_image`), `nostr-smoke` hinter `app()->environment('local')`, Empty-Space-CTA „Zur Startseite". Login-Busy-Labels bereits in D3. Pest deckt Logout-Präsenz + Bottom-Nav-Header + Fehlerseiten-Render + `nostr-smoke`-Gate + OG-Meta (6 Tests). |
