...
(Branch `master`). Der Ordner `packages/nostr-chat/` ist **gitignored** — Composer wählt
die Quelle automatisch (Weiche):

| Situation | Was Composer nimmt |
|---|---|
| `packages/nostr-chat/` liegt als Clone vor | **Symlink** auf den lokalen Ordner (schnelles Dev) |
...
Das Package-Repo in `packages/nostr-chat/` klonen — dann greift der Symlink:

```bash
git clone git@github.com:HolgerHatGarKeineNode/einundzwanzig-group-package.git packages/nostr-chat
...
```bash
cd packages/nostr-chat && git add -A && git commit -m "…" && git push   # → Package-Repo (master)
```
